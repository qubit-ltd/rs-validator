...
注册表仅拥有按稳定 ID 排序的轻量注册值 boxed slice，并通过二分查找定位。注册使用的描述符、签名、依赖规范、ID 和注册来源字符串均为静态值。`BoundValidator` 拥有指向预备实例的 `Arc`，并复制所选静态签名和规则 ID，因此无需重新准备即可克隆。
