...
impl<'a> ArgumentReader<'a> {
    ...
    pub fn finish(&self) -> Result<(), BindError> {
        ...
        {
            return Err(BindError::new(BindErrorKind::UnknownParameter)
                .with_parameter(argument.name()));
        }
        Ok(())
    }
    ...
    fn u32_value(
        ...
        let Some(value) = self.take_optional(name)? else {
            if required {
                return Err(BindError::new(BindErrorKind::MissingParameter)
                    .with_parameter(name));
            }
            return Ok(None);
        };
        let value = match value {
            ...
            ValidationArgument::Integer(value) if value >= 0 => {
                u32::try_from(value as u128)
            }
    ...
    fn take_optional(
        &mut self,
        name: &str,
    ) -> Result<Option<ValidationArgument<'a>>, BindError> {
        let Some(index) = self
            .args
            .iter()
    ...
    fn take(
        &mut self,
        name: &str,
    ) -> Result<ValidationArgument<'a>, BindError> {
        self.take_optional(name)?.ok_or_else(|| {
            BindError::new(BindErrorKind::MissingParameter).with_parameter(name)
        })
    }

    fn type_error(name: &str) -> BindError {
        BindError::new(BindErrorKind::ParameterTypeMismatch)
            .with_parameter(name)
    }
