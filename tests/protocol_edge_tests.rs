...
fn skipped() -> SkippedValidation {
    SkippedValidation::new(
        7,
        ValidationPath::root().with_field("credentials"),
...
fn test_adapter_converts_draft_without_leaking_or_bypassing_report_limits() {
    ...
    assert!(!bounded_report.push_violation(final_violation));
...
fn test_report_limits_accept_exact_capacity_and_reject_one_over() {
    ...
    assert!(violation_report.push_violation(violation()));
    ...
    assert!(skipped_report.push_skipped(skipped()));
